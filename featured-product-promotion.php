 ...
 * Author: wpdevm
 * Description: Enhances your site's functionality by allowing each product to be assigned a unique promotional campaign with a countdown timer feature. This enables active promotion of selected products, drawing customer attention to special offers available for a limited time
 * Version: 1.1
 * Author URI: https://wpdevm.com/
 * Developer: Vladyslav Parshyn
 * Developer URI: https://profiles.wordpress.org/wpdevm/
...
class WC_Featured_Product_Promotion {
	...
	public function woocommerce_missing_notice() {
		echo '<div class="notice notice-error"><p>' . esc_html__( 'WooCommerce is required for this plugin to work. Please install and activate WooCommerce.', 'wc-promoted-product' ) . '</p></div>';
	}
	...
	public function add_settings_nonce( $settings_tabs ) {
		if ( isset( $_GET['page'], $_GET['tab'] ) && $_GET['page'] == 'wc-settings' && $_GET['tab'] == 'featured_product_promotion' ) {
			add_action(
				'woocommerce_settings_tabs_featured_product_promotion',
				function () {
	...
	public function register_settings() {
		...
		add_filter(
			...
			function ( $sections ) {
				$sections['featured_product_promotion'] = esc_html__( 'Featured Product Promotion', 'wc-promoted-product' );
				return $sections;
			}
		);
		...
		add_filter(
			...
			function ( $settings, $current_section ) {
				if ( 'featured_product_promotion' == $current_section ) {
					...
					// Section title
					$custom_settings[] = array(
						'name' => esc_html__( 'Featured Product Promotion Settings', 'wc-promoted-product' ),
						'type' => 'title',
						'desc' => esc_html__( 'Settings for the Featured Product Promotion.', 'wc-promoted-product' ),
						'id'   => 'featured_product_promotion_settings',
					);

					// Field for the promotional product title text
					$custom_settings[] = array(
						'name'     => esc_html__( 'Promotion Title Text', 'wc-promoted-product' ),
						'desc_tip' => esc_html__( 'This will prefix the name of the featured product.', 'wc-promoted-product' ),
						'id'       => 'woocommerce_featured_product_promotion_title_text',
						'type'     => 'text',
						'desc'     => esc_html__( 'Enter the promotion title text.', 'wc-promoted-product' ),
					);

					// Background color picker
					$custom_settings[] = array(
						'name' => esc_html__( 'Background Color', 'wc-promoted-product' ),
						'id'   => 'woocommerce_featured_product_promotion_bg_color',
						'type' => 'color',
						'desc' => esc_html__( 'Choose a background color for the promotion.', 'wc-promoted-product' ),
					);

					// Text color picker
					$custom_settings[] = array(
						'name' => esc_html__( 'Text Color', 'wc-promoted-product' ),
						'id'   => 'woocommerce_featured_product_promotion_text_color',
						'type' => 'color',
						'desc' => esc_html__( 'Choose a text color for the promotion.', 'wc-promoted-product' ),
					);

					$custom_settings[] = array(
						'name'     => esc_html__( 'Enable Countdown Timer', 'wc-promoted-product' ),
						'desc_tip' => esc_html__( 'Enable the countdown timer for a product commercial. Please note that for this you will still need to activate the option in the product editor itself and specify the end time of the promotion', 'wc-promoted-product' ),
						'id'       => 'woocommerce_featured_product_promotion_enable_countdown',
						'type'     => 'checkbox',
						'desc'     => esc_html__( 'Enable to display the countdown timer.', 'wc-promoted-product' ),
					);
	...
	public function add_promotion_product_tab( $tabs ) {
		$tabs['promote_product_tab'] = array(
			'label'  => esc_html__( 'Promote This Product', 'woocommerce' ),
	...
	public function display_featured_product() {
		...
		if ( ! empty( $expiration_date ) && $countdown_enabled ) {
			...
			wp_localize_script(
				'featured-product-promotion-countdown-js',
				'promotionCountdownData',
				array(
	...
	public function enqueue_and_localize_scripts() {
		...
		wp_localize_script(
			'featured-product-promotion-countdown-js',
			'promotionCountdownStrings',
			array(
				'expired' => esc_html__( 'EXPIRED', 'wc-promoted-product' ),
				'days'    => esc_html__( 'days', 'wc-promoted-product' ),
				'hours'   => esc_html__( 'hours', 'wc-promoted-product' ),
				'minutes' => esc_html__( 'minutes', 'wc-promoted-product' ),
				'seconds' => esc_html__( 'seconds', 'wc-promoted-product' ),
			)
		);
	}
	...
	public function load_textdomain() {
		load_plugin_textdomain( 'wc-promoted-product', false, basename( __DIR__ ) . '/languages/' );
	}
}
